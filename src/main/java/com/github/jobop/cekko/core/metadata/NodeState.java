package com.github.jobop.cekko.core.metadata;
...
import com.github.jobop.cekko.enums.ResultEnums;
import com.github.jobop.cekko.enums.RoleEnum;
import lombok.Data;
...
public class NodeState extends LifeCycleAdpter {
    private CekkoConfig config;

    public NodeState(CekkoConfig config) {
        this.config = config;
    }
    ...
    public void init() {
        ...
        if (this.config.getPeers() == null || this.config.getPeerIds() == null) {
            throw new CekkoException(ResultEnums.PEER_OR_PEERID_CANNOT_BE_NULL);
        }
        if (this.config.getPeers().size() != this.config.getPeerIds().size()) {
            throw new CekkoException(ResultEnums.PEER_AND_PEERID_SIZE_NOT_MATCH);
        }
