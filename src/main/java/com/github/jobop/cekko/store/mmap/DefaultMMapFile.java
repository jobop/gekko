...
import com.github.jobop.cekko.utils.MmapUtils;
...
public class DefaultMMapFile extends ShutdownableReferenceCountedResource implements MmapFile {
    ...
    public DefaultMMapFile(String fileName, long fileSize, int osPageSize) {
    ...
    private void forceMkdir(File file) {
        try {
    ...
    public boolean appendMessage(byte[] data, int offset, int length) {
        return (boolean) autoReleaseTemplate(x -> {
